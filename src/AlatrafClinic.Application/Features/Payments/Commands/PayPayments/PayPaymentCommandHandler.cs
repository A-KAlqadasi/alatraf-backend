...
public class PayPaymentCommandHandler : IRequestHandler<PayPaymentCommand, Result<Updated>>
{
    ...
    public PayPaymentCommandHandler(ILogger<PayPaymentCommandHandler> logger, IUnitOfWork uow, IEnumerable<IPaymentTypeHandler> handlers)
    {
        _logger = logger;
        _uow = uow;
        _handlers = handlers;
    ...
    {
        var payment = await _uow.Payments.GetByIdAsync(command.PaymentId, ct);
        if (payment == null)
        {
        ...
        var result = await handler.HandleCreateAsync(payment, typeDto, _uow, ct);
        if (result.IsError)
        {
