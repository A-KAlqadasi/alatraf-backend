...
public class CreateTicketCommandHandler : IRequestHandler<CreateTicketCommand, Result<TicketDto>>
{
    ...
    public CreateTicketCommandHandler(ILogger<CreateTicketCommandHandler> logger, IUnitOfWork uow, HybridCache cache)
    {
        _logger = logger;
        _uow = uow;
        _cache = cache;
    ...
    public async Task<Result<TicketDto>> Handle(CreateTicketCommand command, CancellationToken ct)
    {
        ...
        await _uow.Tickets.AddAsync(ticket.Value, ct);
