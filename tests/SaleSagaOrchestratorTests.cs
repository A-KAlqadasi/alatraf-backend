...
using AlatrafClinic.Application.CommandHandlers;
...
public class SaleSagaOrchestratorTests
{
    ...
    private static async Task<(SaleSagaOrchestrator orchestrator, AlatrafClinicDbContext db, Guid sagaId, StartSaleSagaCommand start, ItemUnit itemUnit, decimal unitPrice)> SeedAsync()
    {
        ...
        await db.SaveChangesAsync(CancellationToken.None);
        var patient = Patient.Create(person.Id, PatientType.Civilian).Value;
        ...
        var diagnosis = Diagnosis.Create(
            ...
            new List<InjuryReason> { reason },
            new List<InjurySide> { side },
            new List<InjuryType> { type },
            patient.Id,
            DiagnosisType.Sales).Value;
        ...
        var orchestrator = new SaleSagaOrchestrator(db, logger);
